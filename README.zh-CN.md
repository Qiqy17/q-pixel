...
- 新增原图对比层，可在拼豆图纸上叠加原图并调节透明度，方便判断网格和细节是否贴合。
- 新增“高保真 / 均衡 / 易制作”三种导入模式：高保真优先还原，均衡轻微去杂色，易制作优先降低色号和制作难度。
- 新增本地智能色号识别，自动锁定疑似轮廓色和高光色，色号优化与原图还原会保护这些关键色。
...
- 自动估算最接近原图像素块的宽高精度，导入后优先生成更贴合的图纸尺寸。
- 采样时更重视单元格中心区域，减少边缘过渡色影响。
- 用局部主色替代简单平均色，避免把两种颜色平均成一个杂色。
- 自动识别边框背景色，将接近背景的浅色噪点合并回背景。
- 导入阶段才进行孤立色块和极小用量杂色清理；后续“轻度还原”不会再次运行这套清理。
- “原图还原”不从已经生成的色号表猜颜色，而是重新读取原图；颜色限制仍由“色号优化”单独控制。
