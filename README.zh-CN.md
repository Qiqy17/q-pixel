...
- 2.0.0 build 9：首页以作品和文件夹为主，提供搜索、排序和四种创建入口；展示图新增摄影材质背景，成品质感 3D 区分不透明白与透明扩展色，并持续标注未实物标定。材质依据见 `docs/material-reference-library.md`。
...
- 导入时会在本机自动判断像素画、插画 / AI 图或照片，并为不同图片采用独立的安全参数；无法确定时允许手动切换。
