...
assert.deepEqual(core.normalizeSettings({ profile: "invalid", exposure: 99, side: "back", background: "transparent" }), { version: 1, profile: "raw", exposure: 2, lightIntensity: 1, lightTemperature: "neutral", side: "back", background: "transparent", showCodes: false, showGrid: true, codeOpacity: 90, codeSize: 50, profileTuning: {}, calibrated: false });
assert.deepEqual(core.tuningFor(core.normalizeSettings({}), "standard"), { textureRoughness: 50, textureOpacity: 75, meltEdge: 58 });
assert.deepEqual(core.normalizeSettings({ profileTuning: { standard: { meltEdge: 130 } } }).profileTuning.standard, { textureRoughness: 50, textureOpacity: 75, meltEdge: 100 });
