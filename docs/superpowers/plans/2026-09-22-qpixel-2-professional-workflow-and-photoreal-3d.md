...
- [ ] 记录当前首页、编辑器、导入、保存、导出、材料预览和拼制导航的可见行为。
- [ ] 为项目 payload 的保存、打开、历史版本和冲突副本补 Node 回归测试。
- [ ] 为当前画布模式、工具选择、右侧面板开关和导入入口补浏览器回归夹具。
- [ ] 保存一组小图、中图和 500×500 稀疏/密集测试工程。
- [ ] 将新增测试加入 `scripts/verify_release.sh`。
- [ ] 运行所有现有测试，确认改版前基线为绿色。
...
- [ ] 建立 `professionalWorkspace`、`patternRebuild`、`photoreal3d` 三个本地功能开关。
- [ ] 默认保持现有稳定界面；开发/测试可通过明确入口开启新工作流。
- [ ] 建立一次性加载模块和资源的状态机，防止重复插入脚本、监听器和 WebGL 上下文。
- [ ] 新模块失败时记录错误并回到现有界面。
- [ ] 添加测试：功能开关关闭时 DOM 和行为与基线一致。
...
- [ ] 将项目 payload 正规化、版本迁移、指纹和冲突判断迁入 `project-model.js`。
- [ ] 将阶段、面板、抽屉和响应式工作区状态迁入 `workspace-state.js`。
- [ ] 将重复的 DOM 显示/隐藏、选中态和安全文本写入迁入 `dom-utils.js`。
- [ ] `web/app.js` 仅调用新模块，不保留同名旧函数副本。
- [ ] 保持旧项目格式可打开，输出 payload 不发生无关变化。
...
- [ ] 重排顶部全局栏：项目、保存、撤销/重做、导入、比较、预览、导出。
- [ ] 增加“设计、配色、拼制、输出、版本”阶段栏。
- [ ] 将现有绘制工具合并为单一固定左工具栏，保留原 ID 或提供兼容映射。
- [ ] 建立中央画布、右侧上下文栏、底部色板的稳定四区布局。
- [ ] 阶段切换只更新上下文内容，不重置画布缩放、平移、选区和当前色。
- [ ] 新框架先在 `professionalWorkspace` 功能开关后运行。
- [ ] 添加测试：五个阶段可切换，当前画布状态不变化。
...
- [ ] 将当前工具属性与当前阶段检查合并为一个上下文侧栏。
- [ ] 建立结构、配色、库存、底板和输出五类检查接口。
- [ ] 检查项返回“通过、警告、错误、可修复动作”，禁止直接修改项目。
- [ ] “全部修复”逐项显示影响范围并统一确认。
- [ ] 删除右侧长列表中已经迁移的重复入口，保留唯一状态源。
...
- [ ] 1180 px 以上显示完整三栏。
- [ ] 768–1179 px 将右侧属性和底部色板改为覆盖抽屉。
- [ ] 小于 768 px 提供兼容布局，不允许弹窗内容被裁切。
- [ ] 高频触控目标最小 44×44 CSS px。
- [ ] 支持鼠标、触控板、触屏和键盘组合输入。
- [ ] 验证横屏/竖屏、浏览器缩放 100% 和系统字体放大。
...
- [ ] 新建项目提供照片转图纸、已有图纸重建、参考图描绘、空白项目。
- [ ] 每个入口显示支持的文件、输出结果和是否改变当前项目。
- [ ] 选择文件后只创建临时导入会话。
- [ ] 继续保留“快速导入，直接编辑”和“高级优化”。
- [ ] 高级优化可跳过、关闭、返回和取消。
- [ ] 入口切换不得遗留上一会话监听器或临时图纸。
...
- [ ] 定义重建输入、校准参数、候选网格、颜色样本和置信度输出格式。
- [ ] 实现边缘裁切、旋转和四角透视校正。
- [ ] 实现横纵周期检测、格距候选、起点和行列数评分。
- [ ] 实现多底板边界候选。
- [ ] 实现单格主色采样、压缩噪声抑制和同色归一。
- [ ] 为每个格子和整体网格返回置信度及原因。
- [ ] 算法保持纯函数；Worker 只负责任务和取消协议。
...
- [ ] 定义品牌、色号、符号、数量和区域的标准图例结构。
- [ ] P0 允许手动录入或粘贴图例映射。
- [ ] 不在 P0 引入未经验证的 OCR 自动应用。
- [ ] 为 P1 保留候选和置信度字段。
